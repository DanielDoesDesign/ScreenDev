 ...
 *  STATIC PROTOTYPES
 **********************/
static void window_create(monitor_t *m, int);
...
static void monitor_sdl_init(void)
{
    ...
    window_create(&monitor, 1);

#if MONITOR_DUAL
    window_create(&monitor2, 2);
    int x, y;
    SDL_GetWindowPosition(monitor2.window, &x, &y);
    SDL_SetWindowPosition(monitor.window, x - (MONITOR_HOR_RES * MONITOR_ZOOM) / 2 - 20, y);
    SDL_SetWindowPosition(monitor2.window, x + (MONITOR_HOR_RES * MONITOR_ZOOM) / 2 + 20, y);
#endif

    sdl_inited = true;
}

static void window_create(monitor_t *m, int ID)
{
    if(ID == 1){
    m->window = SDL_CreateWindow("Player 1",
                                 SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                 MONITOR_HOR_RES * MONITOR_ZOOM, MONITOR_VER_RES * MONITOR_ZOOM, 0); /*last param. SDL_WINDOW_BORDERLESS to hide borders*/
    }

    if(ID == 2){
     m->window = SDL_CreateWindow("Player 2",
                                SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                MONITOR_HOR_RES * MONITOR_ZOOM, MONITOR_VER_RES * MONITOR_ZOOM, 0); /*last param. SDL_WINDOW_BORDERLESS to hide borders*/
    }
